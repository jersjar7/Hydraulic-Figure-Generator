...
export function integer(minimum = Number.NEGATIVE_INFINITY): ValueParser {
  return (value, path) => {
    const result = finite(value, path)
...
export function settings(value: unknown, path: string): ParsedProjectSettings {
  return shape(value, path, {
...
export function annotation(value: unknown, path: string): MapAnnotation | null {
  const input = record(value, path)
  const kind = oneOf(
